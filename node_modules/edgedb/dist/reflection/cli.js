...
const run = async () => {
    ...
    while (args.length) {
        ...
        switch (flag) {
            ...
            case "--tls-security":
                const tlsSec = getVal();
                if (!con_utils_1.validTlsSecurityValues.includes(tlsSec)) {
                    (0, generate_1.exitWithError)(`Invalid value for --tls-security. Must be one of: ${con_utils_1.validTlsSecurityValues
                        .map(x => `"${x}"`)
                        .join(" | ")}`);
                }
...
`);
   ...
   Run \`npx edgedb-generate --help\` for details.`;
    ...
    const outputDir = options.outputDir
        ...
        : adapter_node_1.path.join(projectRoot, "dbschema", "edgeql");
    ...
    if (await (0, adapter_node_1.exists)(outputDir)) {
        if (await canOverwrite(outputDir, options)) {
            await adapter_node_1.fs.rmdir(outputDir, { recursive: true });
        }
    }
    else {
    ...
    if (options.promptPassword) {
        const username = (await (0, con_utils_1.parseConnectArguments)({
            ...connectionConfig,
            password: "",
...
the query builder directory? The following line will be added:
  ...
  ${vcsLine}\n\n`, true)) {
                await adapter_node_1.fs.appendFile(gitIgnorePath, `${vcsLine}\n`);
            }
        }
    }
    process.exit();

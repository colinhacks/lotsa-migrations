...
const generateCastMaps = (params) => {
    ...
    const reverseTopo = Array.from(types)
        .reverse()
        .map(([_, type]) => type);
    const materialScalars = reverseTopo.filter(type => type.kind === "scalar" &&
        !type.is_abstract &&
    ...
    for (const outer of materialScalars) {
        ...
        runtimeMap.writeln([
            (0, builders_1.r) `  if (a.__name__ === ${getRuntimeRef(outer.name)}.__name__) {`,
        ]);
        for (const inner of materialScalars) {
            ...
            if (validCast) {
                ...
                runtimeMap.writeln([
                    (0, builders_1.r) `    if(b.__name__ === ${getRuntimeRef(inner.name)}.__name__) {`,
                ]);
                if (sameType) {

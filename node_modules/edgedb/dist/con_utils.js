...
async function parseConnectArguments(opts = {}) {
    ...
    if (opts.commandTimeout != null) {
        if (typeof opts.commandTimeout !== "number" || opts.commandTimeout < 0) {
            throw new Error("invalid commandTimeout value: " +
...
async function resolveConfigOptions(resolvedConfig, config, sources, compoundParamsError) {
    ...
    anyOptionsUsed =
        resolvedConfig.setPassword((_c = config.password) !== null && _c !== void 0 ? _c : null, sources.password) ||
            anyOptionsUsed;
    anyOptionsUsed =
        (await resolvedConfig.setTlsCAFile((_d = config.tlsCAFile) !== null && _d !== void 0 ? _d : null, sources.tlsCAFile)) || anyOptionsUsed;
    anyOptionsUsed =
        resolvedConfig.setTlsSecurity((_e = config.tlsSecurity) !== null && _e !== void 0 ? _e : null, sources.tlsSecurity) || anyOptionsUsed;
    resolvedConfig.addServerSettings((_f = config.serverSettings) !== null && _f !== void 0 ? _f : {});
    const compoundParamsCount = [
        config.dsn,
        config.instanceName,
        config.credentialsFile,
        (_g = config.host) !== null && _g !== void 0 ? _g : config.port,
    ...
    if (compoundParamsCount === 1) {
        if (config.dsn !== undefined ||
            ...
            if (dsn === undefined) {
                ...
                dsn = `edgedb://${config.host != null ? validateHost(config.host) : ""}`;
            }
            ...
            const creds = await (0, credentials_1.readCredentialsFile)(credentialsFile);
            const source = config.credentialsFile
                ? sources.credentialsFile
                : sources.instanceName;
            resolvedConfig.setHost((_h = creds.host) !== null && _h !== void 0 ? _h : null, source);
            resolvedConfig.setPort((_j = creds.port) !== null && _j !== void 0 ? _j : null, source);
            resolvedConfig.setDatabase((_k = creds.database) !== null && _k !== void 0 ? _k : null, source);
            resolvedConfig.setUser((_l = creds.user) !== null && _l !== void 0 ? _l : null, source);
            resolvedConfig.setPassword((_m = creds.password) !== null && _m !== void 0 ? _m : null, source);
            resolvedConfig.setTlsCAData((_o = creds.tlsCAData) !== null && _o !== void 0 ? _o : null, source);
            resolvedConfig.setTlsSecurity((_p = creds.tlsSecurity) !== null && _p !== void 0 ? _p : null, source);
        }
        return { hasCompoundOptions: true, anyOptionsUsed: true };
    }
    return { hasCompoundOptions: false, anyOptionsUsed };
}
async function parseDSNIntoConfig(dsnString, config, source) {
    let parsed;
    try {
    ...
    await handleDSNPart("tls_cert_file", null, config._tlsCAData, config.setTlsCAFile);

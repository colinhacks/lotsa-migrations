import { ArrayType, BaseType, BaseTypeTuple, cardinalityUtil, LinkDesc, NamedTupleType, ObjectType, PropertyDesc, ScalarType, TupleType, TypeSet } from "../reflection";
...
export declare type assignableBy<T extends BaseType> = T extends ScalarType ? scalarAssignableBy<T> : T extends ObjectType ? anonymizeObject<T> : T extends ArrayType ? ArrayType<assignableBy<T["__element__"]>> : T extends TupleType ? TupleType<assignableTuple<T["__items__"]>> : T extends NamedTupleType ? NamedTupleType<{
    [k in keyof T["__shape__"]]: assignableBy<T["__shape__"][k]>;
}> : never;
export declare type pointerToAssignmentExpression<Pointer extends PropertyDesc | LinkDesc> = [Pointer] extends [PropertyDesc] ? {

import { Cardinality, makeType, TypeKind, cardinalityUtil, } from "../reflection";
import { set } from "./set";
import { isImplicitlyCastableTo } from "@generated/castMaps";
import { literal } from "./literal";
export function $resolveOverload(args, typeSpec, funcDefs) {
    const [positionalArgs, namedArgs] = typeof args[0] === "object" && typeof args[0].__kind__ === "undefined"
        ? [args.slice(1), args[0]]
        : [args, undefined];
    for (const def of funcDefs) {
        const resolvedOverload = _tryOverload(positionalArgs, namedArgs, typeSpec, def);
...
function _tryOverload(args, namedArgs, typeSpec, funcDef) {
    var _a, _b;
...
function compareType(typeSpec, typeId, arg) {
    ...
    if (type.kind === "scalar") {
        return {
            match: arg.__kind__ === TypeKind.scalar &&
